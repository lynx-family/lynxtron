...
export const singleModifierCombinations = modifiers.flatMap((mod) =>
  keyCodes.map((key) => {
    return key === '+' ? `${mod}+Plus` : `${mod}+${key}`;
  })
);

export const doubleModifierCombinations = modifiers.flatMap((mod1, i) =>
  modifiers.slice(i + 1).flatMap((mod2) =>
    keyCodes.map((key) => {
