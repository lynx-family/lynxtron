import { nativeImage } from 'lynxtron';
...
describe('nativeImage module', () => {
  ...
  ifdescribe(process.platform === 'darwin')('isMacTemplateImage state', () => {
    describe('with properties', () => {
      it('correctly recognizes a template image', () => {
        ...
        const templateImage = nativeImage.createFromPath(
          path.join(fixturesPath, 'assets', 'logo_Template.png')
        );
  ...
  describe('createEmpty()', () => {
    it('returns an empty image', async () => {
      ...
      expect(empty.toDataURL({ scaleFactor: 2.0 })).to.equal(
  ...
  describe('createFromBitmap(buffer, options)', () => {
    it('returns an empty image when the buffer is empty', () => {
      expect(
        nativeImage
          .createFromBitmap(Buffer.from([]), { width: 0, height: 0 })
          .isEmpty()
      ).to.be.true();
    });

    it('returns an image created from the given buffer', () => {
      const imageA = nativeImage.createFromPath(
        path.join(fixturesPath, 'assets', 'logo.png')
      );

      const imageB = nativeImage.createFromBitmap(
        imageA.toBitmap(),
        imageA.getSize()
      );
      expect(imageB.getSize()).to.deep.equal({ width: 538, height: 190 });

      const imageC = nativeImage.createFromBuffer(imageA.toBitmap(), {
        ...imageA.getSize(),
        scaleFactor: 2.0,
      });
    ...
    it('throws on invalid arguments', () => {
      expect(() =>
        nativeImage.createFromBitmap(null as any, {} as any)
      ).to.throw('buffer must be a node Buffer');
      expect(() =>
        nativeImage.createFromBitmap([12, 14, 124, 12] as any, {} as any)
      ).to.throw('buffer must be a node Buffer');
      expect(() =>
        nativeImage.createFromBitmap(Buffer.from([]), {} as any)
      ).to.throw('width is required');
      expect(() =>
        nativeImage.createFromBitmap(Buffer.from([]), { width: 1 } as any)
      ).to.throw('height is required');
      expect(() =>
        nativeImage.createFromBitmap(Buffer.from([]), { width: 1, height: 1 })
      ).to.throw('invalid buffer size');
    });
  });
  ...
  describe('createFromBuffer(buffer, options)', () => {
    it('returns an empty image when the buffer is empty', () => {
      expect(
        nativeImage.createFromBuffer(Buffer.from([])).isEmpty()
      ).to.be.true();
    });

    it('returns an empty image when the buffer is too small', () => {
      const image = nativeImage.createFromBuffer(Buffer.from([1, 2, 3, 4]), {
        width: 100,
        height: 100,
      });
    ...
    it('throws on invalid arguments', () => {
      expect(() => nativeImage.createFromBuffer(null as any)).to.throw(
        'buffer must be a node Buffer'
      );
      expect(() =>
        nativeImage.createFromBuffer([12, 14, 124, 12] as any)
      ).to.throw('buffer must be a node Buffer');
    });
  });
  ...
  describe('createFromDataURL(dataURL)', () => {
    ...
    it('returns an image created from the given string', () => {
      for (const imageData of dataUrlImages) {
        ...
        expect(imageFromDataUrl.getSize()).to.deep.equal(
          imageFromPath.getSize()
        );
        expect(imageFromDataUrl.toBitmap()).to.satisfy((bitmap: any) =>
          imageFromPath.toBitmap().equals(bitmap)
        );
        expect(imageFromDataUrl.toDataURL()).to.equal(
          imageFromPath.toDataURL()
        );
      }
    });
  });
  ...
  describe('toDataURL()', () => {
    // FIXME(Guo Xi): fix native image scale factor
    it.skip('returns a PNG data URL', () => {
      for (const imageData of dataUrlImages) {
        const imageFromPath = nativeImage.createFromBuffer(
          nativeImage.createFromPath(imageData.path!).toPNG()
        );
        ...
        for (const scaleFactor of scaleFactors) {
          expect(imageFromPath.toDataURL({ scaleFactor })).to.equal(
            imageData.dataUrl
          );
        }
      }
  ...
  describe('createFromPath(path)', () => {
    it('returns an empty image for invalid paths', () => {
      ...
      expect(
        nativeImage.createFromPath('does-not-exist.png').isEmpty()
      ).to.be.true();
      expect(
        nativeImage.createFromPath('does-not-exist.ico').isEmpty()
      ).to.be.true();
    ...
    it('loads images from paths with `.` segments', () => {
      const imagePath = `${path.join(fixturesPath)}${path.sep}.${
        path.sep
    ...
    it('loads images from paths with `..` segments', () => {
      const imagePath = `${path.join(fixturesPath, 'api')}${path.sep}..${
        path.sep
      }${path.join('assets', 'logo.png')}`;
      const image = nativeImage.createFromPath(imagePath);
      expect(image.isEmpty()).to.be.false();
    ...
    ifit(process.platform === 'darwin')(
      'Gets an NSImage pointer on macOS',
      function () {
        const imagePath = `${path.join(fixturesPath, 'api')}${path.sep}..${
          path.sep
        }${path.join('assets', 'logo.png')}`;
    ...
    ifit(process.platform === 'win32')(
      'loads images from .ico files on Windows',
      function () {
  ...
  describe('createFromNamedImage(name)', () => {
    ...
    ifit(process.platform !== 'darwin')(
      'returns empty on non-darwin platforms',
      function () {
    ...
    ifit(process.platform === 'darwin')(
      'returns a valid image on darwin',
      function () {
        const image = nativeImage.createFromNamedImage('NSActionTemplate');
        expect(image.isEmpty()).to.be.false();
      }
    ...
    ifit(process.platform === 'darwin')(
      'returns allows an HSL shift for a valid image on darwin',
      function () {
        const image = nativeImage.createFromNamedImage('NSActionTemplate', [
          0.5,
          0.2,
  ...
  describe('resize(options)', () => {
    ...
    it('returns an empty image when called on an empty image', () => {
      expect(
        nativeImage.createEmpty().resize({ width: 1, height: 1 }).isEmpty()
      ).to.be.true();
      expect(
        nativeImage.createEmpty().resize({ width: 0, height: 0 }).isEmpty()
      ).to.be.true();
    });
  ...
  describe('crop(bounds)', () => {
    ...
    it('returns an empty image when the bounds are invalid', () => {
      ...
      expect(
        image.crop({ width: 0, height: 0, x: 0, y: 0 }).isEmpty()
      ).to.be.true();
      expect(
        image.crop({ width: -1, height: 10, x: 0, y: 0 }).isEmpty()
      ).to.be.true();
      expect(
        image.crop({ width: 10, height: -35, x: 0, y: 0 }).isEmpty()
      ).to.be.true();
      expect(
        image.crop({ width: 100, height: 100, x: 1000, y: 1000 }).isEmpty()
      ).to.be.true();
    });
  ...
  ifdescribe(process.platform !== 'linux')(
    ...
    () => {
      ...
      it('throws when a bad path is passed', async () => {
        const badPath =
          process.platform === 'win32' ? '\\hey\\hi\\hello' : '/hey/hi/hello';
      ...
      it('returns native image given valid params', async () => {
        ...
        const goodSize = { width: 100, height: 100 };
        const result = await nativeImage.createThumbnailFromPath(
          goodPath,
          goodSize
        );
  ...
  describe('addRepresentation()', () => {
    ...
    // FIXME(Guo Xi): fix native image scale factor
    it.skip('supports adding a buffer representation for a scale factor', () => {
      const image = nativeImage.createEmpty();
      ...
      expect(image.toDataURL({ scaleFactor: 1.0 })).to.equal(
        imageDataOne.dataUrl
      );
      expect(image.toDataURL({ scaleFactor: 2.0 })).to.equal(
        imageDataTwo.dataUrl
      );
      expect(image.toDataURL({ scaleFactor: 3.0 })).to.equal(
        imageDataThree.dataUrl
      );
      expect(image.toDataURL({ scaleFactor: 4.0 })).to.equal(
        imageDataThree.dataUrl
      );
    });

    // FIXME(Guo Xi): fix native image scale factor
    it.skip('supports adding a data URL representation for a scale factor', () => {
      const image = nativeImage.createEmpty();
    ...
    // FIXME(Guo Xi): fix native image scale factor
    it.skip('supports adding a representation to an existing image', () => {
      const imageDataOne = image1x1;
