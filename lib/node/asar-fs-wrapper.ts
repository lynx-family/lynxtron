...
const { assignFunctionName } = __non_webpack_require__(
  'internal/util'
) as typeof import('@node/lib/internal/util');
...
const { URL: NodeURL } = __non_webpack_require__(
  'internal/url'
) as typeof import('@node/lib/internal/url');
...
const createError = (
  errorType: AsarError,
  { asarPath, filePath }: { asarPath?: string; filePath?: string } = {}
) => {
...
function validateBufferIntegrity(
  buffer: Buffer,
  integrity: NodeJS.AsarFileInfo['integrity']
) {
  if (!integrity) return;
  ...
  const actual = crypto
    .createHash(integrity.algorithm)
    .update(buffer)
    .digest('hex');
  if (actual !== integrity.hash) {
    console.error(
      `ASAR Integrity Violation: got a hash mismatch (${actual} vs ${integrity.hash})`
    );
    process.exit(1);
  }
}

const makePromiseFunction = function (
  orig: Function,
...
export const wrapFsWithAsar = (fs: Record<string, any>) => {
  ...
  const logASARAccess = (
    ...
    if (!logFDs.has(asarPath)) {
      ...
      const logPath = path.join(
        (require('os') as typeof os).tmpdir(),
        logFilename
      );
  ...
  const wrapRealpath = function (realpath: Function) {
    return function (
      ...
      realpath(
        asarPath,
        options,
        (error: Error | null, archiveRealPath: string) => {
          if (error === null) {
  ...
  fs.exists[util.promisify.custom] = function exists(pathArgument: string) {
    const pathInfo = splitPath(pathArgument);
    if (!pathInfo.isAsar)
      return nativeExists[util.promisify.custom](pathArgument);
    const { asarPath, filePath } = pathInfo;
  ...
  function readdirRecursive(
    ...
    function read(pathArg: string) {
      ...
      if (pathInfo.isAsar) {
        ...
        if (!readdirResult) {
          const error = createError(AsarError.NOT_FOUND, {
            asarPath,
            filePath,
          });
          nextTick(callback, [error]);
        ...
        if (context.withFileTypes) {
          readdirResult = [
            ...
            readdirResult.map((p: string) => {
              return internalBinding('fs').internalModuleStat(
                path.join(pathArg, p)
              );
            }),
          ];
  ...
  fs.promises.readdir = async function (
    pathArgument: string,
    options: ReaddirOptions
  ) {
    options = getOptions(options);
    ...
    if (options?.withFileTypes) {
      ...
      for (const file of files) {
        ...
        const stats = archive.stat(childPath);
        if (!stats) {
          throw createError(AsarError.NOT_FOUND, {
            asarPath,
            filePath: childPath,
          });
        }
  ...
  modBinding.overrideReadFileSync((jsonPath: string):
    | Buffer
    | false
    | undefined => {
  ...
  async function readdirRecursivePromises(
    originalPath: string,
    options: ReaddirOptions
  ) {
    const result: any[] = [];
  ...
  if (process.platform === 'win32') {
    ...
    fs.mkdirSync = function (pathArgument: string, options: any) {
      const pathInfo = splitPath(pathArgument);
      if (pathInfo.isAsar && pathInfo.filePath.length)
