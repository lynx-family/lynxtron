...
const dialogBinding = process._linkedBinding('lynxtron_binding_dialog');
...
const setupOpenDialogProperties = (
  properties: (keyof typeof OpenFileDialogProperties)[]
): number => {
...
const setupSaveDialogProperties = (
  properties: (keyof typeof SaveFileDialogProperties)[]
): number => {
  let dialogProperties = 0;
  for (const property of properties) {
...
const saveDialog = (
  ...
  return sync
    ? dialogBinding.showSaveDialogSync(settings)
    : dialogBinding.showSaveDialog(settings);
};
...
const openDialog = (
  ...
  return sync
    ? dialogBinding.showOpenDialogSync(settings)
    : dialogBinding.showOpenDialog(settings);
};
...
const messageBox = (
  ...
  if (typeof title !== 'string') throw new TypeError('Title must be a string');
  if (typeof noLink !== 'boolean')
    throw new TypeError('noLink must be a boolean');
  if (typeof message !== 'string')
    throw new TypeError('Message must be a string');
