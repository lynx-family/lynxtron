...
function attemptToMergeAGroup<T>(
  groups: { before?: T[]; after?: T[]; id?: T }[][]
) {
...
function findEdgesInGroup<T>(
  groups: {
    beforeGroupContaining?: T[];
    afterGroupContaining?: T[];
    id?: T;
  }[][],
  i: number,
  edges: Map<any, any>
...
export function sortMenuItems(menuItems: any[]) {
  const isSeparator = (i: any) => {
    const opts = i as any;
    return (
      i.type === 'separator' &&
      !opts.before &&
