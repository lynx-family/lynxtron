...
// Most code came from: chrome/browser/chrome_application_main_posix.cc.
...
// See comment in |WillRunMainMessageLoop()|, where sigaction is called.
...
void MainParts::InstallShutdownSignalHandlers(
    base::OnceCallback<void()> shutdown_callback,
    const scoped_refptr<base::SingleThreadTaskRunner>& task_runner) {
  int pipefd[2];
  int ret = pipe(pipefd);
