...
inline float ForwardFilter(float in_n_1,
  ...
  switch (order) {
    ...
    case RecursiveFilter::FIRST_DERIVATIVE:
      return b[0] * 0.5f * (in_n1 - in_n_1) + b[1] * w[n - 1] +
             b[2] * w[n - 2] + b[3] * w[n - 3];
    case RecursiveFilter::SECOND_DERIVATIVE:
      return b[0] * (in_n - in_n_1) + b[1] * w[n - 1] + b[2] * w[n - 2] +
             b[3] * w[n - 3];
  }
...
inline float BackwardFilter(const std::vector<float>& out,
  ...
  switch (order) {
    case RecursiveFilter::FUNCTION:
    case RecursiveFilter::FIRST_DERIVATIVE:
      return b[0] * w_n + b[1] * out[n + 1] + b[2] * out[n + 2] +
             b[3] * out[n + 3];
    case RecursiveFilter::SECOND_DERIVATIVE:
      return b[0] * (w_n1 - w_n) + b[1] * out[n + 1] + b[2] * out[n + 2] +
             b[3] * out[n + 3];
  }
  NOTREACHED();
  return 0.0f;
  ...
  for (int r = 0; r < row_count;
    ...
    for (; c < row_width - 1; ++c, ++n, byte_index += source_pixel_stride) {
      w[n] = ForwardFilter<order>(
          in[byte_index - source_pixel_stride], in[byte_index],
          in[byte_index + source_pixel_stride], w, n, b);
    }
    ...
    w[n] = ForwardFilter<order>(in[byte_index - source_pixel_stride],
                                in[byte_index], in[byte_index], w, n, b);
    ...
    switch (order) {
      case RecursiveFilter::FUNCTION:
        return SingleChannelRecursiveFilter<RecursiveFilter::FUNCTION, true>(
            source_data, source_pixel_stride, source_row_stride, row_width,
            row_count, output, output_pixel_stride, output_row_stride, b);
      case RecursiveFilter::FIRST_DERIVATIVE:
        return SingleChannelRecursiveFilter<RecursiveFilter::FIRST_DERIVATIVE,
                                            true>(
            source_data, source_pixel_stride, source_row_stride, row_width,
            row_count, output, output_pixel_stride, output_row_stride, b);
      case RecursiveFilter::SECOND_DERIVATIVE:
...
unsigned char SingleChannelRecursiveGaussianX(const unsigned char* source_data,
  ...
  return SingleChannelRecursiveFilter(
      source_data + input_channel_index, input_channel_count,
      source_byte_row_stride, image_size.width(), image_size.height(),
      output + output_channel_index, output_channel_count,
...
unsigned char SingleChannelRecursiveGaussianY(const unsigned char* source_data,
                                              ...
                                              bool absolute_values) {
  return SingleChannelRecursiveFilter(
      source_data + input_channel_index, source_byte_row_stride,
      input_channel_count, image_size.height(), image_size.width(),
      output + output_channel_index, output_byte_row_stride,
