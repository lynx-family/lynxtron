...
const StringMethodPair resize_methods[] = {
    ADD_METHOD(GOOD), ADD_METHOD(BETTER),   ADD_METHOD(BEST),
    ADD_METHOD(BOX),  ADD_METHOD(HAMMING1), ADD_METHOD(LANCZOS3),
};
...
void Benchmark::Usage() {
  printf(
      "image_operations_bench -source wxh -destination wxh "
...
bool Benchmark::Run() const {
  ...
  for (int i = 0; i < num_iterations_; ++i) {
    dest = skia::ImageOperations::Resize(source, method_, dest_.width(),
                                         dest_.height());
  }
...
class CommandLineAutoReset {
 public:
  ...
  ~CommandLineAutoReset() { base::CommandLine::Reset(); }
