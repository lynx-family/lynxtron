...
void CheckResizeMethodShouldAverageGrid(const SkBitmap& src,
  ...
  *method_passed = false;
  ...
  for (size_t pixel_index = 0; pixel_index < std::size(tested_pixels);
    ...
    EXPECT_LE(distance, max_allowed_distance)
        ...
        << ", pixel tested: " << tested_pixel.name << "(" << x << ", " << y
  ...
  for (int y = 0; y < actual_results.height(); y++) {
    for (int x = 0; x < actual_results.width(); x++) {
      ...
      const uint32_t expected_color =
          AveragePixel(src, first_x, last_x, first_y, last_y);
...
TEST(ImageOperations, HalveSubset) {
  ...
  SkBitmap subset_results =
      skia::ImageOperations::Resize(src, skia::ImageOperations::RESIZE_BOX,
...
TEST(ImageOperations, ResizeShouldAverageColors) {
  ...
  SkBitmap src;
  DrawCheckerToBitmap(src_w, src_h, checker_color1, checker_color2,
                      checker_rect_w, checker_rect_h, &src);
  ...
  for (size_t method_index = 0; method_index < std::size(tested_methods);
       ++method_index) {
    bool pass = true;
    CheckResizeMethodShouldAverageGrid(src, tested_methods[method_index],
                                       dest_w, dest_h, average_color, &pass);
    if (!pass) {
...
TEST(ImageOperations, ScaleUp) {
  ...
  SkBitmap dst = skia::ImageOperations::Resize(
      src, skia::ImageOperations::RESIZE_LANCZOS3, dst_w, dst_h);
  ...
  for (int dst_y = 0; dst_y < dst_h; ++dst_y) {
    for (int dst_x = 0; dst_x < dst_w; ++dst_x) {
      ...
      for (int src_y = 0; src_y < src_h; ++src_y) {
        for (int src_x = 0; src_x < src_w; ++src_x) {
          double coeff = lanczos3(src_x + 0.5 - dst_x_in_src) *
