...
SkIPoint MakeImpulseImage(std::vector<unsigned char>* image,
  ...
  const int src_row_stride =
      ComputeRowStride(width, channel_count, stride_slack);
...
SkIRect MakeBoxImage(std::vector<unsigned char>* image,
  ...
  const int src_byte_count = src_row_stride * height;
  const SkIRect box =
      SkIRect::MakeXYWH((width - box_width) / 2, (height - box_height) / 2,
...
TEST(RecursiveGaussian, SmoothingMethodComparison) {
  ...
  MakeImpulseImage(&input, kImgWidth, kImgHeight, kChannelIndex, kChannelCount,
                   kStrideSlack);
  ...
  const int src_row_stride =
      ComputeRowStride(kImgWidth, kChannelCount, kStrideSlack);
  ...
  SingleChannelConvolveX1D(&input[0], src_row_stride, kChannelIndex,
                           kChannelCount, filter, image_size, &intermediate[0],
                           dest_row_stride, 0, 1, false);
  SingleChannelConvolveY1D(&intermediate[0], dest_row_stride, 0, 1, filter,
                           image_size, &control[0], dest_row_stride, 0, 1,
                           false);
  ...
  SingleChannelRecursiveGaussianY(
      &input[0], src_row_stride, kChannelIndex, kChannelCount, recursive_filter,
      image_size, &intermediate2[0], dest_row_stride, 0, 1, false);
  SingleChannelRecursiveGaussianX(&intermediate2[0], dest_row_stride, 0, 1,
                                  recursive_filter, image_size, &output[0],
                                  dest_row_stride, 0, 1, false);
  ...
  for (i1 = control.begin(), i2 = output.begin(); i1 != control.end();
       ++i1, ++i2) {
    if ((*i1 != 0) != (*i2 != 0)) {
      difference_count++;
    }
...
TEST(RecursiveGaussian, SmoothingImpulse) {
  ...
  std::vector<unsigned char> input;
  SkISize image_size = SkISize::Make(kImgWidth, kImgHeight);
  const SkIPoint centre_point =
      MakeImpulseImage(&input, kImgWidth, kImgHeight, kChannelIndex,
  ...
  SingleChannelRecursiveGaussianY(
      &input[0], src_row_stride, kChannelIndex, kChannelCount, recursive_filter,
      image_size, &intermediate[0], dest_row_stride, 0, 1, false);
  ...
  SingleChannelRecursiveGaussianY(
      ...
      image_size, &intermediate[0], dest_row_stride, 0, 1, false);
  SingleChannelRecursiveGaussianX(&intermediate[0], dest_row_stride, 0, 1,
                                  recursive_filter, image_size, &output2[0],
                                  dest_row_stride, 0, 1, false);
  ...
  std::vector<unsigned char>::const_iterator i1, i2;
  int difference_count = 0;
  for (i1 = output.begin(), i2 = output2.begin(); i1 != output.end();
       ++i1, ++i2) {
...
TEST(RecursiveGaussian, FirstDerivative) {
  ...
  const SkIRect box =
      MakeBoxImage(&input, kImgWidth, kImgHeight, kChannelIndex, kChannelCount,
  ...
  SingleChannelRecursiveGaussianX(&input[0], src_row_stride, kChannelIndex,
                                  kChannelCount, recursive_filter, image_size,
                                  &output_x[0], dest_row_stride, 0, 1, true);
  SingleChannelRecursiveGaussianY(&input[0], src_row_stride, kChannelIndex,
                                  kChannelCount, recursive_filter, image_size,
                                  &output_y[0], dest_row_stride, 0, 1, true);
  ...
  int image_total =
      ComputeBoxSum(output, SkIRect::MakeWH(kImgWidth, kImgHeight), kImgWidth);
...
TEST(RecursiveGaussian, SecondDerivative) {
  ...
  int image_total =
      ComputeBoxSum(output, SkIRect::MakeWH(kImgWidth, kImgHeight), kImgWidth);
  int box_inflated = ComputeBoxSum(
      output,
      SkIRect::MakeLTRB(box.left() - spread, box.top() - spread,
                        box.right() + spread, box.bottom() + spread),
      kImgWidth);
  int box_deflated = ComputeBoxSum(
      output,
      SkIRect::MakeLTRB(box.left() + spread, box.top() + spread,
                        box.right() - spread, box.bottom() - spread),
      kImgWidth);
