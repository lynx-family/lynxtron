...
POINT SkPointToPOINT(const SkPoint& point) {
  POINT win_point = {SkScalarRoundToInt(point.fX),
                     SkScalarRoundToInt(point.fY)};
  return win_point;
}
...
void CopyHDC(HDC source,
  ...
  if (is_opaque) {
    BitBlt(destination, x, y, copy_width, copy_height, source, src_rect.left,
           src_rect.top, SRCCOPY);
  } else {
    SkASSERT(copy_width != 0 && copy_height != 0);
    BLENDFUNCTION blend_function = {AC_SRC_OVER, 0, 255, AC_SRC_ALPHA};
    GdiAlphaBlend(destination, x, y, copy_width, copy_height, source,
                  src_rect.left, src_rect.top, copy_width, copy_height,
                  blend_function);
  }
...
HGLOBAL CreateDIBV5ImageDataFromN32SkBitmap(const SkBitmap& bitmap) {
  ...
  if (!base::CheckMul(height, base::CheckMul(width, bpp))
           .AssignIfValid(&bytes)) {
    return nullptr;
  }
