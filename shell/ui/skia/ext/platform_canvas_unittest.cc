...
void MakeOpaque(SkCanvas* canvas, int x, int y, int width, int height) {
  ...
  SkRect rect;
  rect.setXYWH(SkIntToScalar(x), SkIntToScalar(y), SkIntToScalar(width),
...
bool IsOfColor(const SkBitmap& bitmap, int x, int y, uint32_t color) {
  ...
  static uint32_t alpha_mask = static_cast<uint32_t>(SK_A32_MASK)
                               << SK_A32_SHIFT;
...
TEST(PlatformCanvas, TranslateLayer) {
  ...
  canvas->restore();
  EXPECT_TRUE(
      VerifyBlackRect(*canvas, kLayerX + 1, kLayerY + 1, kLayerW, kLayerH));
  ...
  canvas->restore();
  EXPECT_TRUE(
      VerifyBlackRect(*canvas, kInnerX + 1, kInnerY + 1, kInnerW, kInnerH));
  ...
  canvas->restore();
  EXPECT_TRUE(VerifyBlackRect(*canvas, kInnerX + 3, kInnerY + 3, kInnerW - 1,
                              kInnerH - 1));
