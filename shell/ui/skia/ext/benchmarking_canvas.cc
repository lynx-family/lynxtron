...
class FlagsBuilder {
 public:
  FlagsBuilder(char separator) : separator_(separator) {}
...
base::Value AsValue(SkClipOp op) {
  static const char* gOpStrings[] = {
      "Difference", "Intersect",         "Union",
      "XOR",        "ReverseDifference", "Replace"};
...
base::Value AsValue(const SkPath& path) {
  ...
  static const char* gFillStrings[] = {"winding", "even-odd", "inverse-winding",
                                       "inverse-even-odd"};
  ...
  static const char* gVerbStrings[] = {"move",  "line",  "quad", "conic",
                                       "cubic", "close", "done"};
  ...
  static_assert(std::size(gVerbStrings) == std::size(gPtsPerVerb),
                "gPtsPerVerb size mismatch");
  static_assert(std::size(gVerbStrings) == std::size(gPtOffsetPerVerb),
...
void BenchmarkingCanvas::onDrawPoints(PointMode mode,
                                      size_t count,
                                      const SkPoint pts[],
                                      const SkPaint& paint) {
...
void BenchmarkingCanvas::onDrawRRect(const SkRRect& rrect,
                                     const SkPaint& paint) {
...
void BenchmarkingCanvas::onDrawDRRect(const SkRRect& outer,
                                      const SkRRect& inner,
                                      const SkPaint& paint) {
...
void BenchmarkingCanvas::onDrawTextBlob(const SkTextBlob* blob,
                                        SkScalar x,
                                        SkScalar y,
                                        const SkPaint& paint) {
  DCHECK(blob);
