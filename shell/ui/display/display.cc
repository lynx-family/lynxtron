...
Display::Display(int64_t id, const gfx::Rect& bounds)
    : id_(id), bounds_(bounds), work_area_(bounds), device_scale_factor_(1.) {
...
void Display::SetScale(float device_scale_factor) {
  ...
  // Ensure the scale is integral on Apple platforms.
  device_scale_factor = static_cast<int>(device_scale_factor);
#endif
  device_scale_factor_ = std::max(0.5f, device_scale_factor);
}
