...
#include "shell/ui/display/util/display_util.h"
...
void DeIntersectDisplays(int64_t primary_id,
  ...
  std::sort(sorted_displays.begin(), sorted_displays.end(),
            [&](Display* d1, Display* d2) {
              const int d1_depth = GetDisplayTreeDepth(
                  d1->id(), primary_id, display_to_parent_ids_map);
              const int d2_depth = GetDisplayTreeDepth(
                  d2->id(), primary_id, display_to_parent_ids_map);

              if (d1_depth != d2_depth) {
                return d1_depth < d2_depth;
              }

              const int64_t d1_distance =
                  d1->bounds().OffsetFromOrigin().LengthSquared();
              const int64_t d2_distance =
