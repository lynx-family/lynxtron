...
#include "shell/ui/gfx/geometry/vector2d.h"
// #include "shell/ui/gfx/range/range.h"
...
gfx::Rect CoordinateRotateRectangle90(const gfx::Rect& rect) {
  return gfx::Rect(rect.y(), -rect.x() - rect.width(), rect.height(),
                   rect.width());
}
...
gfx::Rect CoordinateRotateRectangle270(const gfx::Rect& rect) {
  return gfx::Rect(-rect.y() - rect.height(), rect.x(), rect.height(),
                   rect.width());
}
...
DisplayPlacement::Position CalculateDisplayPosition(
  ...
  if (max_left == min_right && parent_rect.y() <= current_rect.bottom() &&
    ...
    return parent_rect.x() == max_left ? DisplayPlacement::Position::LEFT
  ...
  if (max_top == min_bottom && parent_rect.x() <= current_rect.right() &&
    ...
    return parent_rect.y() == max_top ? DisplayPlacement::Position::TOP
