...
// #include "shell/ui/gfx/image/image_family.h"
#include "shell/ui/gfx/skbitmap_operations.h"
...
//      image_family.GetBest(IconUtil::kLargeIconSize,
//      IconUtil::kLargeIconSize);
...
//    // Windows' "Large icons" view displays icons at full size only if there
//    is
//    // a 256x256 (kLargeIconSize) image in the .ico file. Otherwise, it
//    shrinks
...
// static
// std::unique_ptr<gfx::ImageFamily>
// IconUtil::CreateImageFamilyFromIconResource(
//    HMODULE module,
//    int resource_id) {
...
//      icon_handle(static_cast<HICON>(LoadImage(
...
void IconUtil::InitializeBitmapHeader(BITMAPV5HEADER* header,
                                      int width,
                                      int height) {
  DCHECK(header);
...
//   // The width field in the BITMAPINFOHEADER structure accounts for the
//   height
//   // of both the AND mask and the XOR mask so we need to multiply the
//   bitmap's
...
//   // object into the appropriate space in the buffer. We use a helper
//   function
//   // (rather than just copying the bits) because there is no way to specify
//   the
//   // orientation (bottom-up vs. top-down) of a bitmap residing in a .ico
//   file.
...
//   // because the underlying image has an alpha channel. An AND mask
//   containing
...
//   CopySkBitmapBitsIntoIconBuffer(unpremul_bitmap, xor_mask_addr,
//   xor_mask_size);
...
void IconUtil::CopySkBitmapBitsIntoIconBuffer(const SkBitmap& bitmap,
  ...
  for (size_t i = 0; i < bitmap_size; i += bitmap.width() * 4) {
    memcpy(buffer + bitmap_size - bitmap.width() * 4 - i, bitmap_ptr + i,
           bitmap.width() * 4);
  }
}
