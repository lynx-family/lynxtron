...
class COMPONENT_EXPORT(GFX) ImageFamily {
 private:
  ...
  struct MapKey : std::pair<float, int> {
    MapKey(float aspect, int width) : std::pair<float, int>(aspect, width) {}
 ...
 public:
  ...
  class COMPONENT_EXPORT(GFX) const_iterator {
   public:
    ...
    const gfx::Image& operator*() const { return map_iterator_->second; }

    const gfx::Image* operator->() const { return &**this; }
