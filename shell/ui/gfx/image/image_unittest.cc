...
TEST_F(ImageTest, EmptyImageFromObviouslyInvalidPNGImage) {
  ...
  image_png_reps2.push_back(
      gfx::ImagePNGRep(new base::RefCountedBytes(), 1.0f));
...
TEST_F(ImageTest, ImageNo1xToPNG) {
  ...
  image_skia.AddRepresentation(
      gfx::ImageSkiaRep(gt::CreateBitmap(kSize2x, kSize2x), 2.0f));
...
TEST_F(ImageTest, MultiResolutionPNGToImageSkia) {
  ...
  EXPECT_TRUE(gt::ArePNGBytesCloseToBitmap(
      *bytes2x, image_skia.GetRepresentation(2.0f).GetBitmap(),
      gt::MaxColorSpaceConversionColorShift()));
  EXPECT_TRUE(
      gt::ImageSkiaStructureMatches(image_skia, kSize1x, kSize1x, scales));
#if !BUILDFLAG(IS_IOS)
...
TEST_F(ImageTest, PNGEncodeFromSkiaDecodeToPlatform) {
  ...
  gfx::Image from_bitmap =
      gfx::Image::CreateFrom1xBitmap(gt::CreateBitmap(25, 25));
...
TEST_F(ImageTest, PNGEncodeFromPlatformDecodeToSkia) {
  ...
  EXPECT_TRUE(gt::AreBitmapsClose(from_platform.AsBitmap(), from_png.AsBitmap(),
                                  gt::MaxColorSpaceConversionColorShift()));
}
...
TEST_F(ImageTest, MultiResolutionImageSkia) {
  ...
  gfx::ImageSkia image_skia;
  image_skia.AddRepresentation(
      gfx::ImageSkiaRep(gt::CreateBitmap(kWidth1x, kHeight1x), 1.0f));
  image_skia.AddRepresentation(
      gfx::ImageSkiaRep(gt::CreateBitmap(kWidth2x, kHeight2x), 2.0f));

  std::vector<float> scales;
  scales.push_back(1.0f);
  scales.push_back(2.0f);
  EXPECT_TRUE(
      gt::ImageSkiaStructureMatches(image_skia, kWidth1x, kHeight1x, scales));
