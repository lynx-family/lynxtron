...
class DynamicSource : public ImageSkiaSource {
 public:
  explicit DynamicSource(const gfx::Size& size)
      : size_(size), last_requested_scale_(0.0f) {}
...
TEST_F(ImageSkiaTest, ManyRepsPerScaleFactor) {
  ...
  image.AddRepresentation(
      ImageSkiaRep(gfx::Size(kSmallIcon1x, kSmallIcon1x), 1.0f));
  image.AddRepresentation(
      ImageSkiaRep(gfx::Size(kSmallIcon2x, kSmallIcon2x), 2.0f));
  image.AddRepresentation(
      ImageSkiaRep(gfx::Size(kLargeIcon1x, kLargeIcon1x), 1.0f));
