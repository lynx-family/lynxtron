...
#include "shell/common/process_singleton/chrome_process_finder.h"
...
bool ParseCommandLine(const COPYDATASTRUCT* cds,
  ...
  if (msg.substr(0, first_null) == L"START") {
    ...
    *parsed_command_line = base::CommandLine::FromString(cmd_line);
    ...
    if (fifth_null == std::wstring::npos || fifth_null == msg.length()) {
...
ProcessSingleton::NotifyResult ProcessSingleton::NotifyOtherProcess() {
  ...
  // TODO(Guo Xi): content::RESULT_CODE_HUNG
  TerminateProcessWithHistograms(process, 2);
...
bool ProcessSingleton::Create() {
  ...
  std::wstring mutexName =
      base::UTF8ToWide("Local\\" + program_name_ + "ProcessSingletonStartup");
    ...
    if (!remote_window_) {
      ...
      if (lock_file_ != INVALID_HANDLE_VALUE) {
        ...
        if (!is_app_sandboxed_) {
          ...
          ::ChangeWindowMessageFilterEx(window_.hwnd(), WM_COPYDATA,
                                        MSGFLT_ALLOW, NULL);
        }
