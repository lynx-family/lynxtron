...
lynxtron_dir = os.path.dirname(os.path.dirname(current_dir))
...
def create_venv(python_bin_path, root_dir):
  ...
  try:
    ...
    cmd = f'"{sys.executable.replace("python3.exe", "python.exe")}" -m venv {venv_path}'
...
def main():
  ...
  parser.add_argument('--root_dir', default=os.path.dirname(lynxtron_dir), help='Root directory for the virtual environment')
