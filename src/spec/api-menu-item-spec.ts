...
describe('MenuItems', () => {
  describe('MenuItem instance properties', () => {
    it('should have default MenuItem properties', () => {
      ...
      expect(item)
        .to.have.property('sublabel')
        .that.is.a('string')
        .equal('goodbye');
      expect(item)
        .to.have.property('accelerator')
        .that.is.a('string')
        .equal('CmdOrControl+Q');
      expect(item).to.have.property('click').that.is.a('function');
      expect(item)
        .to.have.property('enabled')
        .that.is.a('boolean')
        .and.is.true('item is enabled');
      expect(item)
        .to.have.property('visible')
        .that.is.a('boolean')
        .and.is.true('item is visible');
      expect(item)
        .to.have.property('checked')
        .that.is.a('boolean')
        .and.is.false('item is not checked');
      expect(item)
    ...
    it('should have a default accelerator for certain roles', () => {
      const items: Record<string, MenuItem['accelerator']> = {
        ...
        togglefullscreen:
          process.platform === 'darwin' ? 'Control+Command+F' : 'F11',
        window: null,
  ...
  describe('MenuItem.click', () => {
    it('should be called with the item object passed', (done) => {
      ...
      (menu as any)._executeCommand({}, menu.items[0].commandId);
    });
  });
  ...
  describe('MenuItem role execution', () => {
    ...
    it('does not try to execute roles without a valid role property', () => {
      const win = new LynxWindow({ show: false, width: 200, height: 200 });
      const item = new MenuItem({ role: 'asdfghjkl' as any });

      const canExecute = execute(item.role as any, win);
  ...
  describe('MenuItem editMenu', () => {
    it('includes a default submenu layout when submenu is empty', () => {
      ...
      if (process.platform === 'darwin') {
        ...
        expect(item.submenu!.items[10].submenu!.items[0].role).to.equal(
          'showsubstitutions'
        );
        expect(item.submenu!.items[10].submenu!.items[1].type).to.equal(
          'separator'
        );
        expect(item.submenu!.items[10].submenu!.items[2].role).to.equal(
          'togglesmartquotes'
        );
        expect(item.submenu!.items[10].submenu!.items[3].role).to.equal(
          'togglesmartdashes'
        );
        expect(item.submenu!.items[10].submenu!.items[4].role).to.equal(
          'toggletextreplacement'
        );
        expect(item.submenu!.items[11].label).to.equal('Speech');
        expect(item.submenu!.items[11].submenu!.items[0].role).to.equal(
          'startspeaking'
        );
        expect(item.submenu!.items[11].submenu!.items[1].role).to.equal(
          'stopspeaking'
        );
      } else {
  ...
  describe('MenuItem with custom properties in constructor', () => {
    it('preserves the custom properties', () => {
      ...
      expect((menu.items[0].submenu!.items[0] as any).customProp).to.equal(
        'bar'
      );
      expect((menu.items[0].submenu!.items[0] as any).overrideProperty).to.be.a(
        'function'
      );
    });
  });
  ...
  describe('MenuItem accelerators', () => {
    ...
    it('should display modifiers correctly for simple keys', () => {
      ...
      expect((menu as any)._getAcceleratorTextAt(0)).to.equal(
        isDarwin() ? 'Command+A' : 'Ctrl+A'
      );
      expect((menu as any)._getAcceleratorTextAt(1)).to.equal('Shift+A');
    ...
    it.skip('should display correctly for shifted keys', () => {
      ...
      expect((menu as any)._getAcceleratorTextAt(0)).to.equal(`Ctrl+Shift+=`);
      expect((menu as any)._getAcceleratorTextAt(1)).to.equal(`Ctrl+Plus`);
      expect((menu as any)._getAcceleratorTextAt(2)).to.equal(`Ctrl+Shift+3`);
      expect((menu as any)._getAcceleratorTextAt(3)).to.equal(`Ctrl+#`);
      expect((menu as any)._getAcceleratorTextAt(4)).to.equal(`Ctrl+Shift+/`);
      expect((menu as any)._getAcceleratorTextAt(5)).to.equal(`Ctrl+?`);
    });
  });
});
