...
#define HAS_RGBA_SHIFTS(r, g, b, a)                                     \
  (SK_R32_SHIFT == (r) && SK_G32_SHIFT == (g) && SK_B32_SHIFT == (b) && \
   SK_A32_SHIFT == (a))
// For RGBA byte order
#if defined(SK_CPU_LENDIAN) && HAS_RGBA_SHIFTS(0, 8, 16, 24)
...
#else
#error Match Skia's image memory layout.
#endif
#undef HAS_RGBA_SHIFTS
...
CGAffineTransform SkMatrixToCGAffineTransform(const SkMatrix& matrix) {
  ...
  return CGAffineTransformMake(
      matrix[SkMatrix::kMScaleX], matrix[SkMatrix::kMSkewY],
      matrix[SkMatrix::kMSkewX], matrix[SkMatrix::kMScaleY],
      matrix[SkMatrix::kMTransX], matrix[SkMatrix::kMTransY]);
}

SkRect CGRectToSkRect(const CGRect& rect) {
  return SkRect::MakeLTRB(static_cast<SkScalar>(rect.origin.x),
                          static_cast<SkScalar>(rect.origin.y),
                          static_cast<SkScalar>(CGRectGetMaxX(rect)),
                          static_cast<SkScalar>(CGRectGetMaxY(rect)));
}

CGRect SkIRectToCGRect(const SkIRect& rect) {
  CGRect cg_rect;
  cg_rect.origin = CGPointMake(static_cast<CGFloat>(rect.fLeft),
                               static_cast<CGFloat>(rect.fTop));
  cg_rect.size = CGSizeMake(static_cast<CGFloat>(rect.fRight - rect.fLeft),
                            static_cast<CGFloat>(rect.fBottom - rect.fTop));
  return cg_rect;
}
...
SkColor NSSystemColorToSkColor(NSColor* color) {
  ...
  return SkColor4f{static_cast<float>(components[0]),
                   static_cast<float>(components[0]),
                   static_cast<float>(components[0]), static_cast<float>(alpha)}
      .toSkColor();
}
...
SkColor NSDeviceColorToSkColor(NSColor* color) {
  ...
  return SkColor4f{static_cast<float>(red), static_cast<float>(green),
...
SkBitmap NSImageToSkBitmapWithColorSpace(NSImage* image,
                                         bool is_opaque,
                                         CGColorSpaceRef color_space) {
  return NSImageOrNSImageRepToSkBitmapWithColorSpace(image, nil, [image size],
                                                     is_opaque, color_space);
}
...
SkBitmap NSImageRepToSkBitmapWithColorSpace(NSImageRep* image_rep,
                                            ...
                                            bool is_opaque,
                                            CGColorSpaceRef color_space) {
  return NSImageOrNSImageRepToSkBitmapWithColorSpace(nil, image_rep, size,
                                                     is_opaque, color_space);
}
