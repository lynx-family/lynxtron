...
#include "shell/ui/gfx/image/image_skia.h"
#include "shell/ui/gfx/image/image_skia_rep.h"
...
std::string ReadClipboardText(HANDLE handle) {
  ...
  return base::WideToUTF8(data.data());
}
...
std::string ReadClipboardBytes(HANDLE handle) {
  ...
  if (data.data()[actual_size - 1] == '\0') {
    --actual_size;
  }
  return std::string(data.data(), actual_size);
}
...
gfx::Image ReadDib(UINT format) {
  ...
  const BITMAPINFOHEADER* header = dib.data();
...
void Write(const ClipboardData& data) {
  ...
  if (data.text.has_value()) {
    ...
    if (hglb) {
      ...
      if (!dst.data()) {
        ...
        memcpy(dst.data(), w.c_str(), size_bytes);
  ...
  if (data.html.has_value()) {
    ...
    if (hglb) {
      ...
      if (!dst.data()) {
        ...
        memcpy(dst.data(), payload.c_str(), payload.size() + 1);
  ...
  if (data.image.has_value() && !data.image->IsEmpty()) {
    const SkBitmap& bitmap =
        data.image->AsImageSkia().GetRepresentation(1.0f).GetBitmap();
