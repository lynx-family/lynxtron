...
void LynxViewImpl::SetLynxWindow(base::WeakPtr<api::LynxWindow> lynx_window) {
  lynx_window_ = std::move(lynx_window);
}
...
bool LynxViewImpl::StartTestbenchReplay(const std::string& url) {
  ...
  if (!test_bench_replay_controller_) {
    auto resize_callback = [lynx_window = lynx_window_](double width,
                                                        double height) {
      if (lynx_window) {
        lynx_window->ResizeForTestbenchReplay(width, height);
      }
    };
    test_bench_replay_controller_ = std::make_unique<TestbenchReplayController>(
        lynx_view_.get(), std::move(resize_callback));
  }
...
void LynxViewImpl::SendTouchEvent(const std::string& name,
                                  int32_t id,
                                  float x,
                                  float y,
                                  float client_x,
