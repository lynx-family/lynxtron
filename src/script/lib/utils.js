...
function getOutDir (options = {}) {
  ...
  if (options.outDir || process.env.LYNXTRON_OUT_DIR) {
    const outDir = options.outDir || process.env.LYNXTRON_OUT_DIR;
  ...
  // If we got here, it means process.env.LYNXTRON_OUT_DIR was not
  // set and none of the preset options could be found in /out, so throw
  throw new Error(`No valid out directory found; use one of ${presetDirs.join(',')} or set process.env.LYNXTRON_OUT_DIR`);
}
