...
async function runTestUsingElectron (specDir, testName) {
  ...
  if (process.platform === 'win32' && !env.ELECTRON_FORCE_TEST_SUITE_EXIT) {
    env.ELECTRON_FORCE_TEST_SUITE_EXIT = 'true';
  }
  if (process.platform === 'linux') {
    runnerArgs.unshift(path.resolve(__dirname, 'dbus_mock.py'), exe);
    exe = 'python3';
  }
  const { status, signal } = await asyncSpawn(exe, runnerArgs, env);
  if (status !== 0) {
    if (status) {
